...
export const useDashboardStore = defineStore('dashboard', () => {
  ...
  const newServerDefaults = {
    type: 'server', title: '', serverCategory: 'GENERIC',
    link: '', port: '', login: '', password: '',
    customFields: [], isFixed: false, showOnMural: false,
    sites: undefined, // Only for FILE_SERVER
    mxRecords: [], rawMxRecords: '' // Only for EMAIL_SERVICE
  }
  ...
  const newSiteDefaultData = {
    id: null, domainName: '', adminUser: 'preencher', adminPassword: 'preencher',
    associatedDbServerId: '', associatedEmailServiceId: '', rawEmailMxRecords: '',
    associatedReverseProxyId: '', associatedBackupServerId: '',
    customFields: [],
    targetFileServerId: null, originalTargetFileServerId: null, targetFileServerName: ''
  }
  const newSiteData = ref({ ...newSiteDefaultData })

  const emailProviderOptions = ref(['Pixel', 'Heracles', 'Mailgun', 'Zoho Mail', 'Hostinger Mail', 'Outro'])
      ...
      if (Array.isArray(group.sites)) {
        group.sites.forEach(site => {
          ...
          sites.push({
            ...
            groupId: group.id,
            displayBdLink: bdServerDetails ? bdServerDetails.link : (site.associatedDbServerId || 'N/A'),
            displayReverseProxyLink: reverseProxyDetails ? reverseProxyDetails.link : (site.associatedReverseProxyId || 'N/A')
          })
        })
      }
    })
  ...
  const stats = computed(() => {
    if (!allServers.value || !allSitesFlattened.value) return {
      totalFileServers: 0, totalSites: 0, totalBdServers: 0,
      totalReverseProxyServers: 0, totalBackupServers: 0, totalGenericServers: 0,
      totalEmailServicesProviders: 0
    }
    const getCategoryCount = (category) => allServers.value.filter(s => s.serverCategory === category).length
    return {
  ...
  const allSitesFiltered = computed(() => {
    ...
    return allSitesFlattened.value.filter(site =>
      ...
      (site.displayReverseProxyLink && String(site.displayReverseProxyLink).toLowerCase().includes(query))
    )
  })

  const availableFileServers = computed(() => allServers.value.filter(s => s.serverCategory === 'FILE_SERVER'))
  const availableDbServers = computed(() => allServers.value.filter(s => s.serverCategory === 'DB_SERVER'))
  const availableReverseProxyServers = computed(() => allServers.value.filter(s => s.serverCategory === 'REVERSE_PROXY_SERVER'))
  const availableBackupServers = computed(() => allServers.value.filter(s => s.serverCategory === 'BACKUP_SERVER'))
  const availableEmailServices = computed(() => allServers.value.filter(s => s.serverCategory === 'EMAIL_SERVICE'))
  ...
  const selectedEmailServiceForSite = computed(() => {
    if (newSiteData.value.associatedEmailServiceId && availableEmailServices.value.length > 0) {
      return availableEmailServices.value.find(s => s.id === newSiteData.value.associatedEmailServiceId)
    }
    return null
  })
  ...
  async function loadData() {
    console.log('loadData action called')
    isLoading.value = true
    try {
      // TODO: Replace with actual fetch from Cloudflare Function/KV
      const response = await fetch('/sites_data.json') // Fetch from public folder for now
      if (!response.ok) throw new Error(`Failed to fetch sites_data.json: ${response.status}`)
      const data = await response.json()
      if (!Array.isArray(data)) throw new Error('Invalid data format')
      allRawData.value = data
      console.log('Data loaded successfully:', allRawData.value.length, 'items')
    } catch (error) {
  ...
  function viewSiteDetails(siteParam) {
    ...
    if (detailedSite.value.adminPassword && typeof showPasswordsPerSection.value[detailedSite.value.id + '_modal'] === 'undefined') {
  ...
  function openAddServerModal(category = 'GENERIC') {
    ...
    newServerData.value = { ...newServerDefaults, serverCategory: category === 'ALL' ? 'GENERIC' : category, customFields: [] }
  ...
  function openEditServerModal(serverToEdit) {
    ...
    if (newServerData.value.serverCategory === 'FILE_SERVER' && !Array.isArray(newServerData.value.sites)) {
  ...
  function updateNewServerPortDefault(serverDataObj) { // Expects the raw object
    ...
    if (serverDataObj.serverCategory === 'EMAIL_SERVICE') {
      ...
      serverDataObj.rawMxRecords = Array.isArray(serverDataObj.mxRecords) ? serverDataObj.mxRecords.join('\n') : ''
    } else {
      delete serverDataObj.mxRecords
      delete serverDataObj.rawMxRecords
    }
    if (serverDataObj.serverCategory === 'EMAIL_SERVICE' || serverDataObj.serverCategory === 'REVERSE_PROXY_SERVER') {
  ...
  function addServerCustomField() {
    if (!Array.isArray(newServerData.value.customFields)) { newServerData.value.customFields = [] }
  ...
  async function handleSubmitServerForm() {
    ...
    if (newServerData.value.serverCategory !== 'EMAIL_SERVICE' && !newServerData.value.link) missingFields.push('Link/IP')
    if (newServerData.value.serverCategory !== 'EMAIL_SERVICE' && newServerData.value.serverCategory !== 'REVERSE_PROXY_SERVER') {
      if (!newServerData.value.login) missingFields.push('Login')
      if (!newServerData.value.password) missingFields.push('Senha')
    }
    if (missingFields.length > 0) { alert('Preencha: ' + missingFields.join(', ') + '.'); return; }
    ...
    if (serverObjectToSave.serverCategory === 'EMAIL_SERVICE') {
      serverObjectToSave.mxRecords = (serverObjectToSave.rawMxRecords || '').split('\n').map(mx => mx.trim()).filter(mx => mx)
      delete serverObjectToSave.rawMxRecords
      delete serverObjectToSave.port; delete serverObjectToSave.sites
      ...
      else if (serverObjectToSave.serverCategory === 'FILE_SERVER' && !Array.isArray(serverObjectToSave.sites)) serverObjectToSave.sites = []
    }
    ...
    if (isEditModeServer.value) {
      ...
      if (index !== -1) {
        ...
        if (allRawData.value[index].serverCategory === 'FILE_SERVER' && serverObjectToSave.serverCategory === 'FILE_SERVER') {
          serverObjectToSave.sites = allRawData.value[index].sites || []
        }
        allRawData.value.splice(index, 1, serverObjectToSave)
      } else { alert('Erro: Servidor não encontrado para edição.'); return }
    } else {
    ...
    if (isEditModeServer.value && selectedServer.value && selectedServer.value.id === serverObjectToSave.id) {
  ...
  async function confirmDeleteServer(serverToDelete) {
    if (!serverToDelete || !serverToDelete.id) { alert('Servidor inválido.'); return }
  ...
  async function handleDeleteServer(serverId, serverCategory) {
    ...
    if (serverIndex === -1) { alert('Servidor não encontrado.'); return }
    ...
    if (serverCategory !== 'FILE_SERVER') {
      ...
      updatedAllRawData.forEach(fs => {
        if (fs.type === 'server' && fs.serverCategory === 'FILE_SERVER' && Array.isArray(fs.sites)) {
          fs.sites.forEach(site => {
            let siteModifiedInLoop = false
            if (site.associatedDbServerId === serverId) { site.associatedDbServerId = null; siteModifiedInLoop = true }
            if (site.associatedReverseProxyId === serverId) { site.associatedReverseProxyId = null; siteModifiedInLoop = true }
            if (site.associatedBackupServerId === serverId) { site.associatedBackupServerId = null; siteModifiedInLoop = true }
            if (site.associatedEmailServiceId === serverId) { site.associatedEmailServiceId = null; siteModifiedInLoop = true }
  ...
  function openEditSiteModal(siteToEdit) {
      ...
      const parentFileServer = allServers.value.find(s => s.id === parentFileServerId && s.serverCategory === 'FILE_SERVER')
      if (!parentFileServer) {
      ...
      newSiteData.value = {
          ...
          customFields: Array.isArray(siteCopy.customFields) ? JSON.parse(JSON.stringify(siteCopy.customFields)) : [],
  ...
  function addSiteCustomField() {
    if (!Array.isArray(newSiteData.value.customFields)) { newSiteData.value.customFields = [] }
  ...
  async function handleSubmitSiteForm() {
    if (!newSiteData.value.domainName?.trim()) { alert('Domínio é obrigatório.'); return }
    if (!newSiteData.value.targetFileServerId) { alert('File Server de destino não especificado.'); return }
    if (isEditModeSite.value && !newSiteData.value.id) { alert('ID do site ausente para edição.'); return }

    // Determine final MX records
    let finalEmailMxRecords = []
    if (newSiteData.value.associatedEmailServiceId && newSiteData.value.associatedEmailServiceId !== '_ADD_NEW_') {
      ...
      finalEmailMxRecords = (newSiteData.value.rawEmailMxRecords || '').split(',').map(mx => mx.trim()).filter(mx => mx)
    }
    ...
    const targetFileServerIndex = allRawData.value.findIndex(s => s.id === newSiteData.value.targetFileServerId && s.serverCategory === 'FILE_SERVER')
    if (targetFileServerIndex === -1) { alert('File Server de destino não encontrado.'); return }
    ...
    if (!Array.isArray(fileServerToUpdate.sites)) { fileServerToUpdate.sites = [] }
    ...
    if (isEditModeSite.value) {
      ...
      if (siteIndexInTarget !== -1) {
        ...
        fileServerToUpdate.sites[siteIndexInTarget] = { ...fileServerToUpdate.sites[siteIndexInTarget], ...siteEntryData, id: siteIdToUpdate }
      } else {
        // Add site (if moved)
  ...
  async function confirmDeleteSite(siteId, fileServerId) {
    if (!siteId || !fileServerId) { alert('ID do site ou File Server inválido.'); return }
    const fileServer = allRawData.value.find(s => s.id === fileServerId)
    if (!fileServer || !Array.isArray(fileServer.sites)) { alert('File Server não encontrado.'); return }
    const site = fileServer.sites.find(s => s.id === siteId)
    if (!site) { alert('Site não encontrado no File Server.'); return }
  ...
  async function handleDeleteSite(siteId, fileServerId) {
    ...
    if (fileServerIndex === -1) { alert('File Server não encontrado.'); return }

    const fileServerToUpdate = JSON.parse(JSON.stringify(allRawData.value[fileServerIndex]))
    if (!Array.isArray(fileServerToUpdate.sites)) { alert('Estrutura de sites inválida.'); return }

    const siteIndex = fileServerToUpdate.sites.findIndex(s => s.id === siteId)
    if (siteIndex === -1) { alert('Site não encontrado para exclusão.'); return }
  ...
  return {
    // State
    isLoading, currentView, allRawData, globalSearchQuery, siteSearchQuery,
    selectedServerCategoryFilter, selectedServer, detailedSite, showPasswordsPerSection,
    showAddServerModal, isEditModeServer, newServerData,
    showAddSiteModal, isEditModeSite, newSiteData, emailProviderOptions,
    // Getters
    allServers, filteredServers, allSitesFlattened, stats, allSitesFiltered,
    availableFileServers, availableDbServers, availableReverseProxyServers,
    availableBackupServers, availableEmailServices, selectedEmailServiceForSite,
    currentServerCategoryViewTitle,
    // Actions
    loadData, saveData, setView, goHome, selectServerForDetail, viewSiteDetails, closeSiteDetailsModal,
    togglePasswordVisibility, performGlobalSearch, openAddServerModal, openEditServerModal,
    closeAddServerModal, addServerCustomField, removeServerCustomField, handleSubmitServerForm,
    confirmDeleteServer, handleDeleteServer, openAddSiteModal, openEditSiteModal,
    closeAddSiteModal, addSiteCustomField, removeSiteCustomField, handleSubmitSiteForm,
    confirmDeleteSite, handleDeleteSite, getSitesAssociatedWithServer
  }
})
